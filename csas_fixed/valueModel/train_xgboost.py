...
def _dataset_to_numpy(ds, batch_size: int, num_workers: int):
    ...
    X_chunks, y_chunks = [], []
    for x, c, y in loader:
        feats = torch.cat([x, c], dim=1).cpu().numpy().astype(np.float32)  # (B, input+cond)
        target = y.squeeze(-1).cpu().numpy().astype(np.float32)            # (B,)
...
def _predict_with_best_iteration(model: xgb.XGBRegressor, X: np.ndarray) -> np.ndarray:
    """
    Predict using the best_iteration (early stopping) when available.
    Falls back to default prediction otherwise.
    """
    best_iter = getattr(model, "best_iteration", None)
    if best_iter is None:
        return model.predict(X)

    # XGBoost sklearn wrapper supports iteration_range in predict in recent versions.
    # We use [0, best_iter+1) to include best_iter.
    try:
        return model.predict(X, iteration_range=(0, int(best_iter) + 1))
    except TypeError:
        # Fallback: predict via Booster + DMatrix
...
def train(args):
    ...
    ds = ValueDataset(args.stones_csv, args.ends_csv)
    _log(f"Dataset size={len(ds)}", log_path)
    _log(
        f"input_dim={ds.input_dim}, cond_dim={ds.cond_dim}, value_dim=1, num_tasks={ds.num_tasks}",
        log_path,
    )

    group_keys = ds.df[COMPETITION_KEY].to_numpy()
    ...
    if args.val_split > 0.0 and len(val_idx) == 0:
        raise ValueError(
    ...
    if val_idx is not None and len(val_idx) > 0:
        ...
        _log(f"Split: train={len(train_ds)}, val={len(val_ds)}, test={len(test_idx)}", log_path)
    else:
        val_ds = None
        _log(f"Split: train={len(train_ds)}, val=NONE, test={len(test_idx)}", log_path)

    if args.val_competitions_out:
        val_comp_path, n_val_comp = write_split_competition_ids(ds.df, val_idx, args.val_competitions_out)
        _log(
    ...
    if args.test_keys_out:
        test_keys_path, n_test_keys = write_test_shot_keys(ds.df, test_idx, args.test_keys_out)
        _log(
    ...
    if val_ds is not None:
        X_val, y_val = _dataset_to_numpy(val_ds, args.materialize_bs, args.num_workers)
        _log(f"Materialized val: X={X_val.shape}, y={y_val.shape}", log_path)
    else:
        X_val, y_val = None, None

    # Configure early stopping on the estimator (XGBoost 3.x sklearn API).
    est_kwargs = dict(
    ...
    if X_val is not None and args.early_stopping_rounds > 0:
        est_kwargs["early_stopping_rounds"] = args.early_stopping_rounds

    model = xgb.XGBRegressor(**est_kwargs)
    resume_booster = _load_booster_if_exists(args.resume, log_path)

    if X_val is not None:
        _log(
            f"Training with validation. early_stopping_rounds="
            f"{est_kwargs.get('early_stopping_rounds', 0)} (configured on estimator).",
            log_path,
        )
        model.fit(
            X_train,
    ...
    # ---- Report comparable losses (MSE like nn.MSELoss) ----
    # Train loss (optional but useful)
    ...
    # ---- Save model via Booster (avoids sklearn wrapper _estimator_type issue) ----
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train XGBoost value model (real data only).")
    ...
    parser.add_argument("--out", type=str, default="xgb_value_model.json")
    parser.add_argument("--log_file", type=str, default="train_xgb.log")
