...
UNITS_PER_FOOT = (HOGLINE_Y_RAW - BACKLINE_Y_RAW) / 27.0
...
class SynthConfig:
    ...
    team_a_id: int
    team_b_id: int
    out_of_play_prob: float
    radial_scale_mult: float
    jitter_std: float
    out_stones: Path
    out_ends: Path
...
def _append_terminal_row(stone_rows: list[dict], board: np.ndarray, game_id: int, end_id: int, team_id: int, team_order: int, stone_block: int):
    row = {
        "CompetitionID": 0,
        "SessionID": 0,
        "GameID": game_id,
    ...
    for i in range(NUM_STONES):
        row[f"stone_{i+1}_x"] = float(board[i, 0])
        row[f"stone_{i+1}_y"] = float(board[i, 1])
    stone_rows.append(row)


def main():
    ap = argparse.ArgumentParser(description="Generate terminal-only synthetic value-model data.")
    ap.add_argument("--num_games", type=int, default=8192)
    ...
    ap.add_argument("--out_stones", type=str, default="synth_terminal_stones.csv")
    ap.add_argument("--out_ends", type=str, default="synth_terminal_ends.csv")
    args = ap.parse_args()
    ...
    stone_rows: list[dict] = []
    end_rows: list[dict] = []

    for game_id in range(cfg.num_games):
        for end_id in range(1, cfg.ends_per_game + 1):
            board = np.zeros((NUM_STONES, 2), dtype=np.float32)
            first_team_is_a = ((game_id + end_id) % 2 == 1)
            first_team = cfg.team_a_id if first_team_is_a else cfg.team_b_id
            hammer_team = cfg.team_b_id if first_team == cfg.team_a_id else cfg.team_a_id
            ...
            _append_terminal_row(
                stone_rows,
                board=board,
                game_id=game_id,
                end_id=end_id,
                team_id=cfg.team_a_id,
                team_order=int(first_team != cfg.team_a_id),
                stone_block=0,
            )
            _append_terminal_row(
                stone_rows,
                ...
                game_id=game_id,
                end_id=end_id,
                team_id=cfg.team_b_id,
                team_order=int(first_team != cfg.team_b_id),
                stone_block=1,
            )
    ...
    print(f"Saved terminal synth Stones: {cfg.out_stones} ({len(stones_df)} rows)")
    print(f"Saved terminal synth Ends:   {cfg.out_ends} ({len(ends_df)} rows)")
