...
def train(args):
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if val_loader is not None:
            ...
            val_loss = val_running / len(real_val_ds)
    ...
    _log(f"Training finished. Best metric: {best_loss:.4f}. Best checkpoint at {args.out}.", log_path)


if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Train value model on real + synthetic (validate on real only).")
