#!/usr/bin/env python3
"""
Train a Graph Transformer value model (cond_dim=3) for each holdout competition
using the same leakage-free split protocol as `train_holdout_models_cond3.py`:

- test: the held-out competition
- val: a fixed fraction of end-groups from each remaining competition
- train: the rest of the non-held-out real data, plus a configurable synth slice

Checkpoints are written to:
  holdouts/{comp_id}/model_graphtf/model.pt
and include `arch="graph_transformer"` so the scorer can load them directly.
"""
...
from dataset import ValueDataset, NUM_STONES, POS_MAX  # type: ignore
from gnn_models import GNN_REGISTRY  # type: ignore
from train_holdout_models_cond3 import HOLDOUT_IDS, END_KEY, make_holdout_split  # type: ignore
...
def materialize(ds: ValueDataset):
...
def train_one_holdout(args, real_ds, real_Xp, real_Xc, real_Y, synth_Xp, synth_Xc, synth_Y, holdout_comp: int):
    ...
    out_dir = run_dir / "model_graphtf"
    out_dir.mkdir(parents=True, exist_ok=True)
    log_path = Path(args.log_file) if args.log_file else (run_dir / "logs" / "train_graphtf.log")
    if log_path.exists():
        log_path.unlink()
    ...
    # Persist split tables for reproducibility/debugging (same columns as SetTF trainer)
    ...
    ckpt = {
        ...
        "args": {
            ...
            "synth_seed": int(args.synth_seed),
        },
    }
