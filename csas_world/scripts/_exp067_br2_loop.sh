#!/usr/bin/env bash
# EXP-067: BR iteration 2 — league ratchet (best response to az_v26_br2).
...
#   usage: _exp067_stt_loop.sh [N_workers=5] [games_per_worker_per_round=16] [target=600]
set -uo pipefail
cd /mnt/data/curling2/csas_world
N=${1:-3}; G=${2:-16}; TARGET=${3:-600}
OUT=artifacts/replay/mcts/az_v26_br2
WORK=checkpoints/csas_world/az_v26_br2
INC=checkpoints/csas_world/az_v25_br/best.pt   # new champion: opponent, value head, init, gate
CTRL=checkpoints/csas_world/az_v14d/best.pt   # gate opponent = THE INCUMBENT
POL=checkpoints/csas_world/az_v25_br/policy_csas.pt   # NEW champion policy export (iteration-2 self-consistency)
...
echo "[exp067] N=$N G=$G TARGET=$TARGET; start $(date -u +%FT%TZ)" | tee -a "$LOG"

total_games() { ls "$OUT"/r*_shard*.npz 2>/dev/null | wc -l | awk -v g=$G '{print $1*g}'; }

round=1
while true; do
  [ -f "$OUT/STOP" ] && { echo "[exp067] STOP file found" | tee -a "$LOG"; break; }
  done_games=$(total_games)
  [ "$done_games" -ge "$TARGET" ] && { echo "[exp067] target reached: $done_games games" | tee -a "$LOG"; break; }
  R=$(printf "%04d" $round)
  if [ ! -f "$OUT/r${R}_shard0.npz" ]; then
    ...
    for k in $(seq 0 $((N-1))); do
      ...
      env $ENVV JAX_PLATFORMS=cuda XLA_PYTHON_CLIENT_PREALLOCATE=false \
        ...
        --seed $((670000 + round*100 + k)) --scorer bigsel \
...
print(f"[exp067] round $R done: +{tot} records ({act} sig, {act/max(tot,1):.1%}); cumulative games: $(total_games) / $TARGET")
PYEOF
  round=$((round+1))
done

T=artifacts/replay/az_v26_train; V=artifacts/replay/az_v26_val
...
echo "[exp067] split: $(ls $T | wc -l) train / $(ls $V | wc -l) val shards" | tee -a "$LOG"

if [ ! -f "$WORK/best.pt" ]; then
  echo "[exp067] TRAIN start $(date -u +%H:%M)" | tee -a "$LOG"
  unset LD_LIBRARY_PATH
  env $ENVV python3 scripts/run_consolidate.py \
    --config configs/exp_065b_train.yaml --union "$T" --mcts-val "$V" \
    --init "$INC" --out "$WORK" >> "$WORK/train.log" 2>&1
  echo "[exp067] train rc=$?" | tee -a "$LOG"
...
run_eval() {  # outdir, opponent, extra flags...
  local O="eval_out/az_v26_br2/$1"; local OPP="$2"; shift 2
  ...
  echo "[exp067] eval $1 done $(date -u +%H:%M)" | tee -a "$LOG"
}
...
for tag in ("vsinc_k4",):
    W = M = N = 0.0; ms = []
    for f in glob.glob(f"eval_out/az_v26_br2/{tag}/*__h*__s*.json"):
        d = json.load(open(f))
        for k, v in d.items():
...
PYEOF
echo "EXP067_DONE $(date -u +%FT%TZ)" | tee -a "$LOG"
