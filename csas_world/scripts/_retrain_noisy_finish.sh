#!/bin/bash
# Finish the noise-aware retrain: h01-h05 already collected (rollout=5,noise=8);
# collect h06-h10 at rollout=2 (fast) then train the joint model on h01-h10.
set -uo pipefail
...
echo "[$(date +%H:%M:%S)] === finish collection h06-h10 (rollout=2, noise=8) ==="
(
...
echo "[$(date +%H:%M:%S)] === collection complete; tally ==="
