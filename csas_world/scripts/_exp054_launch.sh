#!/usr/bin/env bash
# EXP-054 launcher: 4 resumable shards of the depth x compute dose-response study.
set -uo pipefail
cd /mnt/data/curling2/csas_world
mkdir -p eval_out/exp054_dose
for k in 0 1 2 3; do
  [ "$k" -gt 0 ] && sleep 45
  ...
  nohup python3 scripts/exp054_depth_dose.py --shard-id "$k" --num-shards 4 --device cuda:0 \
    > "eval_out/exp054_dose/shard$k.log" 2>&1 &
  echo "shard $k pid $!"
done
