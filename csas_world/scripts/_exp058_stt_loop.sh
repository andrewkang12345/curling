#!/usr/bin/env bash
# EXP-058: self-consistent StT ecosystem test.
...
#   usage: _exp058_stt_loop.sh [N_workers=5] [games_per_worker_per_round=16] [target=600]
set -uo pipefail
cd /mnt/data/curling2/csas_world
N=${1:-5}; G=${2:-16}; TARGET=${3:-600}
OUT=artifacts/replay/mcts/az_v20_stt
WORK=checkpoints/csas_world/az_v20_stt
INC=checkpoints/csas_world/az_v14d/best.pt
CTRL=checkpoints/csas_world/az_v19_newrules/best.pt
...
echo "[exp058] N=$N G=$G TARGET=$TARGET; start $(date -u +%FT%TZ)" | tee -a "$LOG"

total_games() { ls "$OUT"/r*_shard*.npz 2>/dev/null | wc -l | awk -v g=$G '{print $1*g}'; }

round=1
while true; do
  [ -f "$OUT/STOP" ] && { echo "[exp058] STOP file found" | tee -a "$LOG"; break; }
  done_games=$(total_games)
  [ "$done_games" -ge "$TARGET" ] && { echo "[exp058] target reached: $done_games games" | tee -a "$LOG"; break; }
  R=$(printf "%04d" $round)
  if [ ! -f "$OUT/r${R}_shard0.npz" ]; then
    ...
    for k in $(seq 0 $((N-1))); do
      ...
      env $ENVV timeout 21600 python3 -m world.search.selfplay \
        ...
        --seed $((580000 + round*100 + k)) --scorer stt \
...
print(f"[exp058] round $R done: +{tot} records ({act} sig, {act/max(tot,1):.1%}); cumulative games: $(total_games) / $TARGET")
PYEOF
  round=$((round+1))
done

T=artifacts/replay/az_v20_train; V=artifacts/replay/az_v20_val
...
echo "[exp058] split: $(ls $T | wc -l) train / $(ls $V | wc -l) val shards" | tee -a "$LOG"

if [ ! -f "$WORK/best.pt" ]; then
  echo "[exp058] TRAIN start $(date -u +%H:%M)" | tee -a "$LOG"
  unset LD_LIBRARY_PATH
  env $ENVV python3 scripts/run_consolidate.py \
    --config configs/exp_058_train.yaml --union "$T" --mcts-val "$V" \
    --init "$INC" --out "$WORK" >> "$WORK/train.log" 2>&1
  echo "[exp058] train rc=$?" | tee -a "$LOG"
...
run_eval() {  # outdir, opponent, extra flags...
  local O="eval_out/az_v20_stt/$1"; local OPP="$2"; shift 2
  ...
  echo "[exp058] eval $1 done $(date -u +%H:%M)" | tee -a "$LOG"
}
run_eval vsctrl_k4 "$CTRL" --sel-noise 4

python3 - <<'PYEOF' | tee -a "$LOG" | tee -a experiments_log.md
import json, glob, math
print("\n**EXP-058 raw eval aggregates (auto-appended):**\n")
for tag in ("vsctrl_k4",):
    W = M = N = 0.0; ms = []
    for f in glob.glob(f"eval_out/az_v20_stt/{tag}/*__h*__s*.json"):
        d = json.load(open(f))
        for k, v in d.items():
...
PYEOF
echo "EXP058_DONE $(date -u +%FT%TZ)" | tee -a "$LOG"
