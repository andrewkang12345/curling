#!/usr/bin/env bash
# EXP-063: big-budget self-distillation cycle (certified T2 teacher).
...
#   usage: _exp063_stt_loop.sh [N_workers=5] [games_per_worker_per_round=16] [target=600]
set -uo pipefail
cd /mnt/data/curling2/csas_world
N=${1:-3}; G=${2:-16}; TARGET=${3:-600}
OUT=artifacts/replay/mcts/az_v22_bigsel
WORK=checkpoints/csas_world/az_v22_bigsel
...
echo "[exp063] N=$N G=$G TARGET=$TARGET; start $(date -u +%FT%TZ)" | tee -a "$LOG"

total_games() { ls "$OUT"/r*_shard*.npz 2>/dev/null | wc -l | awk -v g=$G '{print $1*g}'; }

round=1
while true; do
  [ -f "$OUT/STOP" ] && { echo "[exp063] STOP file found" | tee -a "$LOG"; break; }
  done_games=$(total_games)
  [ "$done_games" -ge "$TARGET" ] && { echo "[exp063] target reached: $done_games games" | tee -a "$LOG"; break; }
  R=$(printf "%04d" $round)
  if [ ! -f "$OUT/r${R}_shard0.npz" ]; then
    ...
    for k in $(seq 0 $((N-1))); do
      ...
      env $ENVV timeout 21600 python3 -m world.search.selfplay \
        --config configs/exp_063_bigsel_targets.yaml \
        --games "$G" --num-shards "$N" --shard-id "$k" --split train \
        --seed $((630000 + round*100 + k)) --scorer bigsel \
...
print(f"[exp063] round $R done: +{tot} records ({act} sig, {act/max(tot,1):.1%}); cumulative games: $(total_games) / $TARGET")
PYEOF
  round=$((round+1))
done

T=artifacts/replay/az_v22_train; V=artifacts/replay/az_v22_val
...
echo "[exp063] split: $(ls $T | wc -l) train / $(ls $V | wc -l) val shards" | tee -a "$LOG"

if [ ! -f "$WORK/best.pt" ]; then
  echo "[exp063] TRAIN start $(date -u +%H:%M)" | tee -a "$LOG"
  unset LD_LIBRARY_PATH
  env $ENVV python3 scripts/run_consolidate.py \
    --config configs/exp_063_train.yaml --union "$T" --mcts-val "$V" \
    --init "$INC" --out "$WORK" >> "$WORK/train.log" 2>&1
  echo "[exp063] train rc=$?" | tee -a "$LOG"
...
run_eval() {  # outdir, opponent, extra flags...
  local O="eval_out/az_v22_bigsel/$1"; local OPP="$2"; shift 2
  ...
  echo "[exp063] eval $1 done $(date -u +%H:%M)" | tee -a "$LOG"
}
...
for tag in ("vsinc_k4",):
    W = M = N = 0.0; ms = []
    for f in glob.glob(f"eval_out/az_v22_bigsel/{tag}/*__h*__s*.json"):
        d = json.load(open(f))
        for k, v in d.items():
...
PYEOF
echo "EXP063_DONE $(date -u +%FT%TZ)" | tee -a "$LOG"
