#!/usr/bin/env bash
# H2 A/B: exp_044 (mix_value 0.30) on the L4 leg's accumulated data, gated h2h vs v13it1.
# Compare against iter2's existing exp_043 result (h2h ds -0.032 +/- 0.032).
set -uo pipefail
cd /mnt/data/curling2/csas_world
WORK=checkpoints/csas_world/az_v15_ab30
...
echo "[ab] train exp_044 on L4-leg accum data" | tee -a "$LOG"
unset LD_LIBRARY_PATH
env $ENVV python3 scripts/run_consolidate.py \
  --config configs/exp_044_mixvalue_ab.yaml \
...
echo "[ab] train rc=$?" | tee -a "$LOG"
CK="$WORK/best.pt"; [ -f "$CK" ] || CK="$WORK/model.pt"
for d in 1 2 3; do
  OUT=eval_out/az_v15_ab30/run$d
  ...
  echo "[ab] draw $d done" | tee -a "$LOG"
done
echo "AZ_V15_AB_DONE $(date -u +%H:%M)" | tee -a "$LOG"
