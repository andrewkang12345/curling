...
class LocalNoise:
    ...
    def sample_batch(self, centers: np.ndarray, n: int) -> np.ndarray:
        ...
        if str(b.get("distribution", "gaussian")).lower() == "student_t":
            ...
            z = self.rng.standard_t(df=nu, size=(len(flat), int(n), 4)).astype(np.float32)
            ...
            out = flat[:, None, :] + self.rng.normal(0.0, std, size=(len(flat), int(n), 4)).astype(np.float32)
